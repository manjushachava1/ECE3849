...
static const char * const gTimeScaleStr[TIME_SCALE_STEPS] = {
		" 20 us", " 50 us", "100 us", "200 us",
		"500 us", "  1 ms", "  2 ms", "  5 ms",
		" 10 ms", " 20 ms", " 50 ms", "100 ms"
};

// frequency scale display strings
static const char * const gFreqScaleStr[TIME_SCALE_STEPS] = {
        "20 kHz", "7.8kHz", "3.9kHz", " 2 kHz",
        "781 Hz", "391 Hz", "195 Hz", "78.1Hz",
        "39.1Hz", "19.5Hz", "7.81Hz", "3.91Hz"
};
...
static const char * const gVoltageScaleStr[VOLTAGE_SCALE_STEPS] = {
...
void OscilloscopeDrawGrid(tContext *psContext)
{
	...
	for (i = (FRAME_SIZE_Y / 2) % PIXELS_PER_DIV; i < FRAME_SIZE_Y; i += PIXELS_PER_DIV)
	    GrLineDrawH(psContext, 0, FRAME_SIZE_X - 1, i);
	for (i = (FRAME_SIZE_X / 2) % PIXELS_PER_DIV; i < FRAME_SIZE_X; i += PIXELS_PER_DIV) {
	    if (i == FRAME_SIZE_X/2)
...
void OscilloscopeDrawSettings(tContext *psContext)
{
    ...
    tRectangle rect = {gAdjusting * 8*6, 0, gAdjusting * 8*6 + 6*6 - 1, 7};
	...
	GrStringDraw(psContext, gTimeScaleStr[gTimeSetting], /*length*/ -1, /*x*/ 0, /*y*/ 0, /*opaque*/ false);
	// voltage scale string
	GrStringDraw(psContext, gVoltageScaleStr[gVoltageSetting], /*length*/ -1, /*x*/ 8*6, /*y*/ 0, /*opaque*/ false);
...
void OscilloscopeSpectrumDrawSettings(tContext *psContext)
{
    ...
    GrStringDraw(psContext, gFreqScaleStr[gTimeSetting], /*length*/ -1, /*x*/ 0, /*y*/ 0, /*opaque*/ false);
    // voltage scale string
    GrStringDraw(psContext, "20 dBV", /*length*/ -1, /*x*/ 8*6, /*y*/ 0, /*opaque*/ false);
}
...
int OscilloscopeTrigger(void)	// search for the trigger
{
	...
	if (gTriggerRising) {
	    for (i = 0; i < ADC_BUFFER_SIZE/2; i++, y_last = y) {
	        ...
	        if (y <= gTriggerLevel && y_last > gTriggerLevel) break;
	    }
	}
	else { // falling slope
	    for (i = 0; i < ADC_BUFFER_SIZE/2; i++, y_last = y) {
...
void WaveformTask(UArg arg1, UArg arg2)
{
    ...
    while (true) {
        ...
        if (!gSpectrumMode) { // time domain mode
            uint32_t trigger_start = Timestamp_get32();
            trigger_index = OscilloscopeTrigger();
            OscilloscopeCopyWaveform(trigger_index - FRAME_SIZE_X/2, FRAME_SIZE_X); // copy triggered waveform to a local buffer
...
void DisplayTask(UArg arg1, UArg arg2)
{
    ...
    while (true) {
        ...
        Semaphore_pend(WaveformBufferMutex, BIOS_WAIT_FOREVER);
        OscilloscopeDrawWaveform(&sContext);
        Semaphore_post(WaveformBufferMutex);

        // CPU load measurement
        //        count_loaded = cpu_load_count();
        //        snprintf(str, sizeof(str), "CPU load = %.1f%%", 100.0f * (1.0f - (float)count_loaded / count_unloaded));
        //        GrContextForegroundSet(&sContext, ClrWhite);
        //        GrStringDraw(&sContext, str, /*length*/ -1, /*x*/ 0, /*y*/ FRAME_SIZE_Y-8, /*opaque*/ false);

        // debugging: draw the state of the buttons
        //        for (i = 0; i < 9; i++) strButtons[i] = (gButtons & (1 << i)) ? '1' : '0';
        //        GrContextForegroundSet(&sContext, ClrRed);
        //        GrStringDraw(&sContext, strButtons, /*length*/ 9, /*x*/ 0, /*y*/ 10, /*opaque*/ false);
