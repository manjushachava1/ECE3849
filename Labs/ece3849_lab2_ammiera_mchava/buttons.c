...
int bButtonPressed = 0; // keeps track of if buttons have been pressed
...
void ButtonInit(void)
{
    ...
    // GPIO PJ0 and PJ1 = EK-TM4C1294XL buttons SW2 SW1
...
uint32_t ButtonAutoRepeat(void)
{
    ...
    uint32_t mask;
    uint32_t presses = 0;
    for (i = 0; i < BUTTON_AND_JOYSTICK_COUNT; i++) { // for as many relevant buttons on the board
        mask = 1 << i;
        if (gButtons & mask) // if the button is pressed
...
void ButtonISR(void) {
    ...
    if ((presses & 0x04) == 0x04) { // S1 pressed
        fifo_put(S1);
        bButtonPressed = 1;
    }

    if ((presses & 0x08) == 0x08) { // S2 pressed
        fifo_put(S2);
        bButtonPressed = 1;
    }
}
